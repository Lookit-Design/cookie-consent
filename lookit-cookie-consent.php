...
add_action( 'admin_init', function () {
    register_setting( 'lookit_cc_settings', 'lookit_cc_options', [ 'sanitize_callback' => 'lookit_cc_sanitize' ] );
} );

function lookit_cc_sanitize( $input ) {
    $clean = [];
    $clean['enabled']              = ! empty( $input['enabled'] ) ? 1 : 0;
    $clean['display_style']        = in_array( $input['display_style'] ?? '', [ 'panel', 'card', 'pill' ], true ) ? $input['display_style'] : 'panel';
    ...
    $clean['position']             = in_array( $input['position'] ?? '', [ 'bottom-left', 'bottom-right', 'bottom-center', 'top-center' ] ) ? $input['position'] : 'bottom-left';
...
function lookit_cc_get_options() {
    ...
    return wp_parse_args( get_option( 'lookit_cc_options', [] ), $defaults );
}
...
function lookit_cc_ajax_record() {
    ...
    $preferences = [ 'cookie_consent' => $accepted ];
    $allowed_purposes = [
        'functionality', 'experience', 'measurement', 'marketing',
        'sale_of_personal_information', 'sharing_of_personal_information', 'targeted_advertising'
    ];
    ...
    if ( ! empty( $policy_id ) ) {
        $payload['legal_notices'] = [
            [ 'identifier' => 'cookie_policy' ],
            [ 'identifier' => 'privacy_policy' ],
        ];
    }

    $response = wp_remote_post( 'https://consent.iubenda.com/public/consent', [
        'headers' => [
            'ApiKey'       => $public_key,
    ...
    if ( is_wp_error( $response ) ) {
        ...
        wp_send_json_success( [ 'warning' => 'API call failed but consent cookie was set' ] );
    }
    ...
    if ( $code >= 200 && $code < 300 ) {
        ...
        wp_send_json_success( [ 'warning' => 'API returned ' . $code ] );
    }
}
                ...
                <tr>
                    ...
                    <td>
                        <select id="lookit_cc_position" name="lookit_cc_options[position]">
                            <?php foreach ( [ 'bottom-left' => 'Bottom Left', 'bottom-right' => 'Bottom Right', 'bottom-center' => 'Bottom Center', 'top-center' => 'Top Center' ] as $val => $label ) {
                ...
                <tr>
                    ...
                    <td>
                        <select id="lookit_cc_display_style" name="lookit_cc_options[display_style]">
                            <?php foreach ( [ 'panel' => 'Panel — full popup (default)', 'card' => 'Small card — compact, buttons stacked', 'pill' => 'Corner pill — tiny strip, expands on Customize' ] as $val => $label ) {
...
function lookit_cc_output() {
    ...
    #lookit-cc-popup {
        ...
        <?php switch ( $pos ) {
            case 'bottom-right':  echo 'bottom:24px!important;right:24px!important;left:auto!important;top:auto!important;transform:none!important;'; break;
            case 'bottom-center': echo 'bottom:24px!important;left:50%!important;right:auto!important;top:auto!important;transform:translateX(-50%)!important;'; break;
            case 'top-center':    echo 'top:24px!important;left:50%!important;right:auto!important;bottom:auto!important;transform:translateX(-50%)!important;'; break;
            default:              echo 'bottom:24px!important;left:24px!important;right:auto!important;top:auto!important;transform:none!important;';
        } ?>
